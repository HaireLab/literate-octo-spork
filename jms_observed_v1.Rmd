...
#### Buffer the point locations of actual salamander detections from the historical detection database and extract canopy cover, CWD, and other variables of interest from the 'contemporary' climate scenario/replicates. These variables will form the set of predictors for modeling. A separate model will be constructed for each simulation year, across replicats.

### There is an exploratory step here, in which we graph the distribution of variables within the buffers....

#### The locations within the buffers represent presence. A set of random points represent absence. Using these data, we can model predicted probabilities for JMS and then generate spatial predictions for the Jemez landscape. 

#### Graph the results of the spatial predictions in geographic space to explore the distribution of habitat based on the models. 
...
### Step 3
### Construct a dataset for predictive models
#### Extract values from the point buffers and combine across the 10 replicates for each year.Add a set of random points outside the buffers for modeling.  
...
#### Use generalized boosted regression to predict probability of occurrence for each year. 
...
### Step 5
### Quantify spatial patterns of habitat
...
### Step 6
### Quantify habitat parameters in data space
